...
Self-hosted on Cloudflare Workers + D1 + R2 (or Node + Postgres + S3 via the [storage interface](https://github.com/obs-unified/obs-unified/blob/main/rfcs/0008-storage-interface.md)). Ships traces, logs, metrics, session replay (rrweb), AI/LLM observability, product analytics, alerts, and profiles in one stack with a [Connected rail](/docs/what-to-expect#the-connected-rail) that exposes every cross-signal pivot in ≤2 clicks. Free; you pay your own infra bill.
