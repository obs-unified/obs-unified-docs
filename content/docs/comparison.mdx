...
<Callout type="info" title="Reviewed 2026-05-19 · Next review 2026-08-19">
...
Self-hosted on Cloudflare Workers + D1 + R2 (or Node + Postgres + S3 via the [storage seam](https://github.com/obs-unified/obs-unified/blob/main/rfcs/0008-storage-seam.md)). Ships traces, logs, metrics, session replay (rrweb), AI/LLM observability, product analytics, alerts, and profiles in one stack with a [Connected rail](/docs/what-to-expect#the-connected-rail) that exposes every cross-signal pivot in ≤2 clicks. Free; you pay your own infra bill.
...
- **obs-unified** deliberately does **not** use ClickHouse. The default backend is **SQLite via Cloudflare D1**, with a Postgres adapter via the [storage seam](https://github.com/obs-unified/obs-unified/blob/main/rfcs/0008-storage-seam.md). The trade-off: D1 caps at ~100M hot rows per project (then archive sweep or move to Postgres), in exchange for zero operational cost on the Workers tier and a single-binary local deploy.
...
obs-unified's positioning is the bet that the *graph shape* (one identity chain, one store, ≤2-click pivots) is the right organizing structure for that convergence — not that any specific vendor is wrong. Most of the vendors above will persist; the design hypothesis is just that convergence rewards graph shape over suite shape, and that "we own our data plane" is a multiplier on that.
...
- If you want **everything-in-one (traces, logs, metrics, replay, LLM, analytics) self-hosted on your infra with ≤2-click cross-signal pivots** → **obs-unified**, with the trade-offs in the next section.
