...
This page walks through wiring `@obsunified/analytics-sdk` and `@obsunified/telemetry-sdk` into a real two-tier app — a React frontend that calls a Worker backend — so a single user click produces a usage event, a span, a log, and (optionally) an AI call, all carrying the same `interaction_id`.
...
```ts
import { withChildSpan, createLogger } from "@obsunified/telemetry-sdk";
...
```ts
import { setAISessionContext } from "@obsunified/telemetry-sdk";

setAISessionContext({ sessionId, userId });

const response = await openai.chat.completions.create({ /* ... */ });
// Your typed helper emits a span
