...
All four packages live in the obs-unified monorepo. The TypeScript packages publish to public npm under `@obs-unified/*`. The Go SDK is consumed from the public `sdks/go` module path through Go module tags like `sdks/go/v0.1.0`. The Rust SDK publishes to crates.io as `obs-unified`.
