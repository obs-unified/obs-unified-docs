...
| Package | Where it runs | What it does |
| --- | --- | --- |
| `@obs-unified/mcp-server` | Local MCP host over stdio | Exposes read-only investigation tools for status, recent traces, trace detail, service maps, logs, AI sessions, users, replays, profiles, evals, connected signals, agent runs, actions, and tool calls |
