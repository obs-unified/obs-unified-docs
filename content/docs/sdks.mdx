---
title: SDKs
description: First-party SDKs for TypeScript, Go, and Rust on the backend; a browser SDK for usage and replay. They share one identity-propagation contract.
---
...
| --- | --- | --- |
| **TypeScript** | `pnpm add @obs-unified/telemetry-sdk` | Node.js · Bun · Deno · Cloudflare Workers |
...
| LLM span | `withLLMSpan(opts, fn)` | `obs.WithLLMSpan(ctx, opts, fn)` | `with_llm_span(opts, fn).await` |
| Tool span | `withToolSpan(opts, fn)` | `obs.WithToolSpan(ctx, opts, fn)` | `with_tool_span(opts, fn).await` |
| Project ID | `setProjectId(id)` | `obs.SetProjectID(ctx, id)` | `set_project_id(id)` |
...
All four packages live in the obs-unified monorepo. The TypeScript packages publish to npm under `@obs-unified/*`; the Go SDK is consumed straight from the `sdks/go` module path; the Rust crate publishes to crates.io as `obs-unified`.
