...
| --- | --- | --- |
| `@obs/analytics-sdk` | Browser (React or vanilla) | Tracks usage events, identifies users, mints **interaction_id** on every click, auto-injects `x-obs-interaction` on outbound `fetch`/XHR |
| `@obs/telemetry-sdk` | Node.js / Cloudflare Workers / Bun | Creates OTLP spans, captures logs and AI calls, reads the inbound `x-obs-interaction` header and stamps it on the active span |

Both packages live in the obs-unified monorepo (`packages/analytics-sdk` and `packages/telemetry-sdk`). They're published under the `@obs/*` scope.
...
```tsx
import { AnalyticsProvider, AnalyticsErrorBoundary } from "@obs/analytics-sdk/react";
...
```ts
import { createLogger } from "@obs/telemetry-sdk";
...
```ts
import { startToolSpan, startRetrieverSpan, setAISessionContext } from "@obs/telemetry-sdk";
