...
When a request comes from a browser instrumented with `@obsunified/analytics-sdk`, the request carries `x-obs-interaction`. Copy that value onto the active span as `obs.interaction.id`.
