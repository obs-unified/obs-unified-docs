...
The MCP server lives in the obs-unified monorepo at
`packages/mcp-server`. GitHub Packages publishing is pending a package-write
token, so use a local checkout today:
...
Once the package is published to the obs-unified GitHub Packages registry, it
