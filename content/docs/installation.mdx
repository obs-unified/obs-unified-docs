...
This page covers cloning and running the **obs-unified monorepo** — the collector, dashboard, and TypeScript SDKs. If you just want to instrument a service with one of the published SDKs, jump to the [SDKs page](/docs/sdks) instead; TypeScript packages are published through GitHub Packages, while Go and Rust use the public Git repository.

## Prerequisites (for the monorepo)

- **Node.js 22+** and **pnpm 10+** — required to build the collector, dashboard, and `@obs-unified/*` TypeScript packages
...
> The Go and Rust SDKs are separate Go/Cargo workspaces under `sdks/go` and `sdks/rust`. Working on those only needs the respective language toolchain (Go 1.22+ / Rust 1.75+), not Node.
...
The repo is a pnpm monorepo with eight workspace packages and four apps. `pnpm install` wires them together via the workspace protocol — no `npm link` needed.
